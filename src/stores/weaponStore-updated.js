...
export const useWeaponStore = defineStore('weapon', () => {
  ...
  const isWeaponAvailableForTemplate = (weapon, template) => {
    // Check requirements (template must have at least one required tag)
    if (weapon.requirements && weapon.requirements.length > 0) {
      if (!template.tags || !template.tags.some(tag => weapon.requirements.includes(tag))) {
        return false
      }
    }
    
    // Check restrictions (template must not have any restricted tag)
    if (weapon.restrictions && weapon.restrictions.length > 0) {
      if (template.tags && template.tags.some(tag => weapon.restrictions.includes(tag))) {
        return false
      }
    }
