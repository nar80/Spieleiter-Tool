<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      ...
      <q-space />
      <q-btn flat round dense icon="file_download" @click="handleExport">
    ...
    <q-tab-panels v-model="tab" animated>
      ...
      <q-tab-panel name="players">
        <div class="row q-gutter-md q-mb-md">
          <q-btn
            ...
            icon="add"
            label="Neuer Spieler"
            @click="openTemplateDialog('player')"
          />
        </div>

        <div class="row q-col-gutter-md">
          <div
            v-for="template in playerTemplates"
            :key="template.id"
          ...
          >
            <q-card>
              ...
              <q-card-section class="q-pt-none">
                ...
                <div
                  v-if="
                    (template.weaponInstances &&
                      template.weaponInstances.length) ||
                    (template.weapons && template.weapons.length)
                  "
                >
              ...
              <q-card-actions>
                ...
                <q-btn
                  ...
                  icon="shield"
                  color="info"
                  @click="openArmorDialog(template)"
                >
      ...
      <q-tab-panel name="npcs">
        ...
        <div class="row q-col-gutter-md">
          <div
            v-for="template in filteredNPCTemplates"
            :key="template.id"
            class="col-12 col-md-6 col-lg-4"
          >
            <q-card>
        ...
        <div
          v-if="filteredNPCTemplates.length === 0"
          class="text-center text-grey q-pa-xl"
        >
          {{
            searchQuery || selectedTags.length > 0 || selectedCategory
              ? "Keine NPCs gefunden"
              : "Keine NPC-Templates vorhanden"
          }}
        </div>
      </q-tab-panel>
      ...
      <q-tab-panel name="groups">
          ...
          <q-item v-for="group in characterStore.playerGroups" :key="group.id">
            <q-item-section avatar>
              <q-radio
                ...
                :val="group.id"
                @update:model-value="setActiveGroup"
              />
            </q-item-section>
        ...
        <div
          v-if="characterStore.playerGroups.length === 0"
          class="text-center text-grey q-pa-xl"
        >
        ...
        <q-list bordered separator>
          ...
          >
            <q-item-section>
              ...
              <div class="q-mt-sm">
                <q-chip
                  v-for="(entry, index) in getEncounterDisplayEntries(
                    encounter
                  )"
                  :key="index"
                  size="sm"
          ...
          <q-tab-panel name="basic">
            <div class="q-gutter-sm">
              ...
              <div class="row q-gutter-sm">
                <q-input
                  v-model.number="templateForm.hp"
                  label="HP"
                  type="number"
                  filled
                  class="col"
              ...
              <div v-if="systemStore.selectedSystem === 'wh40k'">
                ...
                <div class="row q-gutter-sm q-mb-sm">
                  <q-input
                    v-model.number="templateForm.attributes.KG"
                    label="KG (Kampfgeschick)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.BF"
                    label="BF (Ballistik)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.ST"
                    label="ST (Stärke)"
                    type="number"
                    filled
                    class="col"
                ...
                <div class="row q-gutter-sm q-mb-sm">
                  <q-input
                    v-model.number="templateForm.attributes.WI"
                    label="WI (Widerstand)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.GE"
                    label="GE (Gewandtheit)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.IN"
                    label="IN (Intelligenz)"
                    type="number"
                    filled
                    class="col"
                  />
                ...
                <div class="row q-gutter-sm">
                  <q-input
                    v-model.number="templateForm.attributes.WA"
                    label="WA (Wahrnehmung)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.WK"
                    label="WK (Willenskraft)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.CH"
                    label="CH (Charisma)"
                    type="number"
                    filled
                    class="col"
              ...
              <div v-if="systemStore.selectedSystem === 'dnd5e'">
                ...
                <div class="row q-gutter-sm q-mb-sm">
                  <q-input
                    v-model.number="templateForm.attributes.STR"
                    label="STR (Stärke)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.DEX"
                    label="DEX (Geschick)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.CON"
                    label="CON (Konstitution)"
                    type="number"
                    filled
                    class="col"
                  />
                ...
                <div class="row q-gutter-sm">
                  <q-input
                    v-model.number="templateForm.attributes.INT"
                  ...
                  />
                  <q-input
                    v-model.number="templateForm.attributes.WIS"
                    label="WIS (Weisheit)"
                    type="number"
                    filled
                    class="col"
                  />
                  <q-input
                    v-model.number="templateForm.attributes.CHA"
                    label="CHA (Charisma)"
                    type="number"
                    filled
                    class="col"
          ...
          <q-tab-panel name="talents">
            <div class="q-gutter-sm">
              ...
              >
                ...
                <div
                  v-if="
                    talent.stackable && selectedTalentIds.includes(talent.id)
                  "
                  class="q-ml-lg"
                >
                  <q-slider
              ...
              <div class="q-mb-sm">
                ...
                <q-list
                  ...
                  v-if="
                    ...
                    templateForm.weaponInstances.length > 0
                  "
                >
                  <q-item
                    v-for="(weapon, index) in templateForm.weaponInstances"
      ...
      <q-card style="min-width: 600px; max-width: 800px">
        ...
        <q-card-section style="max-height: 60vh" class="scroll">
          <div v-if="armorTemplate">
            <div class="q-mb-md">
              ...
              <div
                v-if="
                  !armorTemplate.armorIds || armorTemplate.armorIds.length === 0
                "
                class="text-grey"
              >
            ...
            <div
              v-for="(categoryArmors, category) in armorStore.armorsByCategory"
              :key="category"
            >
              <q-expansion-item
                ...
                <q-list separator>
                  ...
                  >
                    <q-item-section>
                      ...
                      <q-item-label caption>
                        Bedeckt:
                        {{ armorStore.formatCoverage(armor.coverage) }} | RP:
                        {{ armor.rp }}
                      </q-item-label>
                    </q-item-section>
                    <q-item-section side>
        ...
        <q-card-section>
          ...
          <div class="row q-gutter-sm q-mt-xs">
            <q-chip
              v-for="part in ['head', 'torso', 'arms', 'legs']"
              :key="part"
            >
    ...
    <q-dialog v-model="encounterDialog" persistent>
      <q-card style="min-width: 600px; max-width: 900px">
        <q-card-section>
          <div class="text-h6">
            {{ editingEncounter ? "Encounter bearbeiten" : "Neues Encounter" }}
          </div>
        </q-card-section>
        ...
        <q-card-section style="max-height: 70vh" class="scroll">
          ...
          <div
            v-if="
              encounterForm.npcEntries && encounterForm.npcEntries.length > 0
            "
            class="q-mb-md"
          >
            ...
            <q-list bordered separator dense>
              <q-item
                v-for="(entry, index) in encounterForm.npcEntries"
                :key="index"
              >
                <q-item-section>
                  <q-item-label>
                    {{ entry.count }}x
                    {{
                      characterStore.templates.find((t) => t.id === entry.npcId)
                        ?.name || "Unbekannt"
                    }}
                  </q-item-label>
                </q-item-section>
          ...
          <div class="row q-col-gutter-sm">
            <div
              v-for="npc in filteredEncounterNPCs"
              :key="npc.id"
              ...
              <q-card flat bordered>
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label caption>
                      <q-badge
                        :color="getCategoryColor(npc.category)"
                        size="sm"
                      >
                        {{ npc.category }}
                      </q-badge>
                      <span
                        v-if="npc.tags && npc.tags.length > 0"
                        class="q-ml-sm"
                      >
                        <q-chip
          ...
          <div
            v-if="filteredEncounterNPCs.length === 0"
            class="text-center text-grey q-pa-md"
          >
            {{
              encounterSelectedTags.length > 0
                ? "Keine NPCs mit diesen Tags gefunden"
        ...
        <q-card-section>
          ...
          <div class="q-mt-md">
            <q-radio
              v-model="importMode"
              val="merge"
              label="Zu bestehenden Templates hinzufügen"
            />
            <q-radio
              v-model="importMode"
              val="replace"
              label="Alle Templates ersetzen"
              color="red"
            />
          </div>
        </q-card-section>
...
const tagOptions = computed(() => {
  ...
  const allTags = [...new Set([...existingTags, ...commonTags])].filter(
...
const handleExport = () => {
  ...
  if (result.success) {
    const fileName = `spielleiter-templates-${
...
const resetTemplates = () => {
  $q.dialog({
    ...
    message:
      "ACHTUNG: Dies löscht ALLE aktuellen Templates, Gruppen und Encounters und lädt die 10 Standard WH40k Templates. Fortfahren?",
    cancel: true,
    persistent: true,
    ok: {
      label: "Zurücksetzen",
...
const handleImport = () => {
  ...
  if (result.success) {
    ...
    if (result.groupsImported > 0)
      message += ` und ${result.groupsImported} Gruppen`;
    if (result.encountersImported > 0)
      message += ` und ${result.encountersImported} Encounters`;
    message += " importiert";
...
const editEncounter = (encounter) => {
  ...
  if (encounter.npcEntries && encounter.npcEntries.length > 0) {
    ...
    encounterForm.npcEntries = encounter.npcIds.map((id) => ({
      npcId: id,
      count: 1,
    }));
  } else {
...
const decreaseNpcCount = (index) => {
  if (
    encounterForm.npcEntries[index] &&
    encounterForm.npcEntries[index].count > 1
  ) {
    encounterForm.npcEntries[index].count--;
  }
};
...
const getEncounterDisplayEntries = (encounter) => {
  ...
  if (encounter.npcEntries && encounter.npcEntries.length > 0) {
    ...
    encounter.npcEntries.forEach((entry) => {
      const template = characterStore.templates.find(
        (t) => t.id === entry.npcId
      );
      if (template) {
        const displayText =
          entry.count > 1 ? `${entry.count}x ${template.name}` : template.name;
    ...
    Object.entries(npcCounts).forEach(([id, count]) => {
      ...
      if (template) {
        const displayText =
          count > 1 ? `${count}x ${template.name}` : template.name;
        entries.push(displayText);
      }
    });
  }
