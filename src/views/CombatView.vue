<template>
  <q-page class="q-pa-md">
        ...
        >
          <q-list separator>
            <q-item
              ...
              :class="{
                'bg-amber-10':
                  combatStore.isInCombat &&
                  combatStore.currentCombatant?.id === combatant.id,
            ...
            >
              <q-item-section avatar>
                ...
                >
                  ...
                  <q-badge floating color="dark" text-color="white">{{
              ...
              <q-item-section>
                <q-item-label>
                  ...
                  >
                    <q-tooltip
                      >Greift an:
                      {{ getTargetName(combatant.targetId) }}</q-tooltip
                    >
                  </q-icon>
                  <q-badge
                    v-if="combatant.hp.current <= 0"
                    color="red"
                    class="q-ml-sm"
                  >
                    Kampfunfähig
                  ...
                  <q-chip
                    v-if="combatant.currentCover"
                    size="sm"
                    color="brown"
                    text-color="white"
                    class="q-ml-sm"
                  >
                    {{ combatant.currentCover.label }}:
                    {{
                      combatant.currentCover.value -
                      (combatant.currentCover.damage || 0)
                    }}
                    <q-icon
                      v-if="combatant.currentCover.damage > 0"
                      name="warning"
                      size="xs"
                      class="q-ml-xs"
                    />
          ...
          <div
            v-if="playerCombatants.length === 0"
          ...
          <q-list separator>
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label
                  v-if="
                    combatant.engagedWith && combatant.engagedWith.length > 0
                  "
                  caption
                >
                  <div class="row q-gutter-xs q-mt-xs">
                    <div
                      v-for="engagement in combatant.engagedWith"
                      :key="engagement.playerId"
                      class="row items-center q-gutter-xs"
                    >
                      <q-avatar
                        size="20px"
                        :color="getPlayerColor(engagement.playerId)"
                        text-color="white"
                      >
                        <q-icon
                          size="xs"
                          :name="getPlayerIcon(engagement.playerId)"
                        />
                      </q-avatar>
                      <span class="text-caption"
                        >{{ engagement.distance || "?" }}m</span
                      >
                      <q-btn
                        flat
                        round
    ...
    <div
      v-if="combatStore.isInCombat"
      class="row q-gutter-sm justify-center q-mb-md"
    >
      <q-btn
        color="primary"
        icon="skip_previous"
    ...
    <q-card
      v-if="combatStore.isInCombat && combatStore.currentCombatant"
      class="q-mb-md"
    >
      <q-card-section>
        <div class="text-h6 q-mb-md">
          {{
            combatStore.currentCombatant.type === "player"
              ? "Spieler"
              : "Aktionen"
          }}: {{ combatStore.currentCombatant.name }}
          <q-badge
            :color="
              combatStore.currentCombatant.type === 'player' ? 'green' : 'red'
            "
            class="q-ml-sm"
          >
            {{
          ...
          <div class="row q-gutter-md q-mb-md">
            ...
            <div class="col-auto">
              ...
              <div class="q-gutter-xs">
                <div>
                  Rüstung: {{ combatStore.currentCombatant.armor || 0 }}
                </div>
                <div>
                  Widerstand: {{ combatStore.currentCombatant.toughness || 0 }}
                </div>
                <div
                  v-if="combatStore.currentCombatant.criticalWounds"
                  class="text-red"
                >
                  Kritische Wunden:
                  {{ combatStore.currentCombatant.criticalWounds }}
                </div>
              </div>
            </div>
            ...
            <div class="col">
                ...
                >
                  ...
                  <q-btn-group flat>
                    ...
                    <q-btn
                      ...
                      size="sm"
                      :label="`Salve (${
                        weaponStore.parseFireModes(weapon.rof).burst || 3
                      })`"
                      @click="
                        usePlayerAmmo(
                          index,
                    ...
                    <q-btn
                      ...
                      size="sm"
                      :label="`Voll (${
                        weaponStore.parseFireModes(weapon.rof).full || 10
                      })`"
                      @click="
                        usePlayerAmmo(
                          index,
                    ...
                    <q-btn
                      ...
                      :disable="
                        !weapon.mag || weapon.currentAmmo === weapon.mag
                      "
                    >
              ...
              <div v-else class="text-grey">Keine Waffen mit Munition</div>
            </div>
          </div>
        ...
        <!-- NPC Panel - Aktionen -->
        <div v-else>
          <!-- Werte und Talente in einer Row -->
          <div class="row q-gutter-md q-mb-md">
            <!-- Alle Attribute -->
            ...
            <div
              v-if="
                combatStore.currentCombatant?.talents &&
                combatStore.currentCombatant.talents.length > 0
              "
            ...
            >
              ...
              <div class="row q-col-gutter-xs">
                <div
                  v-for="talent in getCurrentCombatantTalents()"
                  :key="talent.id"
                  class="col-6"
                >
                  <q-card
                    flat
                    bordered
                    class="cursor-pointer talent-card q-pa-xs"
                    @click="showTalentDialog(talent)"
              ...
              >
                +{{ getCurrentCombatantTalents().length - 8 }} weitere...
              </div>
            </div>
          </div>
          ...
          <q-list separator>
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label caption>
                  HP: {{ template.hp?.max || template.hp }} | Init:
                  {{ template.initiativeModifier }}
                  <span
                    v-if="template.armorIds && template.armorIds.length > 0"
                  >
                    | {{ template.armorIds.length }} Rüstungsteil(e)</span
                  >
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label caption>
                  ...
                  {{ template.hp?.max || template.hp }} | Init:
                  {{ template.initiativeModifier }}
                  <span
                    v-if="template.armorIds && template.armorIds.length > 0"
                  >
    ...
    <q-dialog v-model="showEncounterSelector">
      <q-card style="min-width: 500px; max-width: 800px">
        ...
        <q-card-section style="max-height: 60vh" class="scroll">
            ...
            >
              <q-item-section>
                ...
                <div class="q-mt-sm">
                  ...
                  >
                    {{
                      characterStore.templates.find((t) => t.id === npcId)
                        ?.name || "Unbekannt"
                    }}
                  </q-chip>
                </div>
              ...
              <q-item-section side>
                <q-badge color="red"
                  >{{ encounter.npcIds.length }} NPCs</q-badge
                >
              </q-item-section>
            </q-item>
          </q-list>
          ...
          <div
            v-if="characterStore.encounterGroups.length === 0"
            class="text-center text-grey q-pa-xl"
          >
    ...
    <q-dialog v-model="attackDialog">
      <q-card
        style="width: 85vw; max-width: 1200px; height: 85vh; max-height: 800px"
      >
        <q-card-section class="q-pb-none">
        ...
        <q-card-section class="scroll" style="max-height: calc(90vh - 120px)">
          ...
          <div class="q-mb-md">
            ...
            <div class="row q-gutter-md q-mb-md">
              <div class="col-auto">
                ...
                <div class="q-gutter-sm">
                  <q-checkbox
                    v-model="attackModifiers.aim"
                    label="Zielen (+10)"
                    dense
                  />
                  <q-checkbox
                    v-model="attackModifiers.shortRange"
                    label="Kurze Reichweite (+10)"
                    dense
                  />
                  <q-checkbox
                    v-model="attackModifiers.targetProne"
                    label="Ziel liegend (-10)"
                    dense
                  />
                  <q-checkbox
                    v-model="attackModifiers.running"
                    label="Laufen (-20)"
                    dense
                  />
                </div>
            ...
            <!-- Berechnung -->
            <div
              class="row items-center q-gutter-md bg-grey-10 q-pa-sm rounded-borders"
            >
              <div>
              ...
              <div v-if="attackModifiers.targetProne" class="text-orange">
                Ziel liegend: -10
              </div>
              <div v-if="attackModifiers.running" class="text-red">
                Laufen: -20
              </div>
              <div v-if="selectedFireMode === 'burst'" class="text-green">
                Salve: +10
              </div>
              <div v-if="selectedFireMode === 'full'" class="text-green">
                Vollautomatik: +20
              </div>
              <div
                v-if="attackModifiers.custom !== 0"
                :class="attackModifiers.custom > 0 ? 'text-green' : 'text-red'"
              >
                Sonstige: {{ attackModifiers.custom > 0 ? "+" : ""
                }}{{ attackModifiers.custom }}
              </div>
              <q-space />
          ...
          <div class="q-mb-md">
            <q-btn
              color="primary"
              label="Angriffswurf (W100)"
              @click="rollAttack"
            />
            <div v-if="attackRoll" class="q-mt-sm">
              <div
                class="text-h4"
                :class="attackHit ? 'text-green' : 'text-red'"
              >
                Wurf: {{ attackRoll }}
              </div>
              <div v-if="attackHit">
          ...
          <div v-if="attackHit" class="q-mb-md">
            ...
            <div
              ...
              :key="index"
              class="q-mb-md q-pa-md bg-grey-10 rounded-borders"
            >
              <!-- Treffer-Header -->
              <div class="row items-center q-mb-sm">
                ...
                <div class="text-subtitle1">
                  ...
                  <span class="text-caption text-grey q-ml-sm">
                    ({{ hit.originalRoll }} → {{ hit.reversedRoll
                    }}{{ index > 0 ? "+" + index * 10 : "" }} = {{ hit.roll }})
                  </span>
                </div>
              </div>
                ...
                <div v-if="hit.damage" class="col">
                  ...
                  <div class="text-caption text-grey">
                    ...
                    <span v-if="hit.furyDamageRolls?.length > 0">
                      | Zorn: +{{
                        hit.furyDamageRolls.reduce((sum, f) => sum + f.total, 0)
                      }}
                    </span>
                  </div>
                </div>
              ...
              <div
                v-if="
                  hit.damage?.hasCritical &&
                  systemStore.selectedSystem === 'wh40k'
                "
              ...
              >
                <q-banner class="bg-amber-10 text-white">
                  ...
                  <div>
                    Natürliche
                    {{ hit.damage.diceSize === 5 ? "5" : "10" }} gewürfelt!
                  </div>
                </q-banner>
                ...
                <div class="q-mt-sm">
                  ...
                  <div v-if="hit.righteousFuryRoll" class="q-mt-sm">
                    <div
                      :class="
                        hit.righteousFuryHit ? 'text-positive' : 'text-negative'
                      "
                    >
                      Wurf: {{ hit.righteousFuryRoll }} (benötigt
                      {{ calculatedHitChance }}%)
                    </div>
                    <div
                      v-if="hit.righteousFuryHit && !hit.righteousFuryConfirmed"
                    >
                      <q-btn
                        color="amber"
                        size="sm"
                ...
                <div v-if="hit.furyDamageRolls?.length > 0" class="q-mt-sm">
                  ...
                  <div
                    v-for="(fury, fIndex) in hit.furyDamageRolls"
                    :key="fIndex"
                  >
                    +{{ fury.total }} ({{ fury.rolls.join("+")
                    }}{{ fury.modifier >= 0 ? "+" : "" }}{{ fury.modifier }})
                    <span v-if="fury.hasCritical" class="text-amber">
                      → Weitere 10!</span
                    >
                  </div>
                </div>
    ...
    <q-dialog v-model="diceDialog">
      <q-card style="min-width: 300px">
        ...
        <q-card-section>
          <div class="row q-gutter-sm">
            <q-btn
              v-for="dice in [4, 6, 8, 10, 12, 20, 100]"
              :key="dice"
              :label="`W${dice}`"
              color="primary"
              @click="rollDice(dice)"
    ...
    <q-dialog v-model="damageDialog">
      <q-card style="min-width: 300px">
        <q-card-section>
          <div class="text-h6">
            {{ damageMode === "damage" ? "Schaden" : "Heilung" }}
          </div>
        </q-card-section>
        ...
        <q-card-section>
          <q-input
            ...
            :label="
              damageMode === 'damage' ? 'Schadenspunkte' : 'Heilungspunkte'
            "
            type="number"
            filled
            autofocus
    ...
    <q-dialog v-model="defenseDialog">
      <q-card style="width: 600px; max-width: 90vw">
        <q-card-section class="q-pb-none">
          <div class="text-h5">
            Verteidigung: {{ defendingCombatant?.name }}
          </div>
        </q-card-section>
        ...
        <q-card-section>
          ...
          <div v-if="defendingCombatant?.type === 'player'">
            ...
            <div class="q-mb-md">
              ...
              <div class="row q-gutter-md items-center q-mb-sm">
                ...
                <q-btn
                  v-if="
                    defendingCombatant.currentCover &&
                    defendingCombatant.currentCover.value > 0
                  "
                  flat
                  dense
                  size="sm"
              ...
              <div
                v-if="
                  ...
                  defendingCombatant.currentCover.value > 0
                "
                class="q-pa-md bg-brown-10 rounded-borders"
              >
                <div class="text-h6 q-mb-sm">
                  {{ defendingCombatant.currentCover.label }}
                </div>
                <div class="row items-center q-gutter-md">
                  <div>
                    <div>
                      Basis-Schutz:
                      <strong
                        >{{ defendingCombatant.currentCover.value }} RP</strong
                      >
                    </div>
                    <div
                      v-if="defendingCombatant.currentCover.damage > 0"
                      class="text-orange"
                    >
                      Schaden erlitten:
                      <strong
                        >-{{
                          defendingCombatant.currentCover.damage
                        }}
                        RP</strong
                      >
                    </div>
                    <div class="text-h5 q-mt-sm">
                      ...
                      <strong
                        >{{
                          defendingCombatant.currentCover.value -
                          (defendingCombatant.currentCover.damage || 0)
                        }}
                        RP</strong
                      >
                    </div>
            ...
            <div class="q-mb-md">
              ...
              <div v-if="defenseType !== 'none'" class="q-mb-md">
                ...
                <div v-if="defenseRoll" class="q-mt-sm">
                  <div
                    class="text-h5"
                    :class="defenseSuccess ? 'text-green' : 'text-red'"
                  >
                    Wurf: {{ defenseRoll }} (benötigt
                    {{ defenseSkill + defenseModifier }}%)
                  </div>
                  <div v-if="defenseSuccess" class="text-positive">
                    ERFOLGREICH
                    {{ defenseType === "dodge" ? "AUSGEWICHEN" : "PARIERT" }}!
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="q-mb-md" v-if="defendingCombatant?.type != 'player'">
            ...
            <div
              v-if="
                ...
                defendingCombatant.currentCover.value > 0
              "
              class="q-pa-sm bg-brown-10 rounded-borders"
            >
              <div class="text-caption">
                <strong>{{ defendingCombatant.currentCover.label }}</strong>
                <div>
                  Basis-Schutz: {{ defendingCombatant.currentCover.value }} RP
                </div>
                <div
                  v-if="defendingCombatant.currentCover.damage > 0"
                  class="text-orange"
          ...
          >
            ...
            <div class="q-gutter-xs">
              <div>
                Eingehender Schaden: <strong>{{ incomingDamage }}</strong>
              </div>
              <div>
                Durchschlag: <strong>{{ incomingPen }}</strong>
              </div>
              <div
                v-if="
                  hitZoneInCover && defendingCombatant?.currentCover?.value > 0
                "
              >
              ...
              <div>
                Rüstung: <strong>-{{ effectiveArmorForHitZone }}</strong>
              </div>
              <div>
                Widerstandsbonus: <strong>-{{ toughnessBonus }}</strong>
              </div>
        ...
        <q-card-actions align="right">
          <q-btn
            ...
            label="Schließen"
            v-close-popup
            v-if="defendingCombatant?.type === 'player'"
          />
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-section>
          <div class="text-subtitle2 q-mb-sm">
            Wähle Spieler und Entfernungen:
          </div>
          <q-list>
            <q-item v-for="player in playerCombatants" :key="player.id">
              <q-item-section avatar>
                <q-checkbox
                  ...
                  @update:model-value="
                    (val) => handleEngagementChange(player.id, val)
                  "
                />
              </q-item-section>
              <q-item-section avatar>
              ...
              <q-item-section side>
                <q-input
                  ...
                  @update:model-value="
                    updateEngagementDistance(player.id, $event)
                  "
                  type="number"
                  suffix="m"
                  filled
    ...
    <q-dialog v-model="initiativeDialog">
      <q-card style="min-width: 400px">
        <q-card-section>
          ...
          <div class="text-caption">
            {{ systemStore.initiativeDice() === "d10" ? "W10" : "W20" }} +
            Modifier
          </div>
        </q-card-section>
          ...
          <q-list>
            <q-item v-for="player in playerInitiatives" :key="player.id">
              ...
              <q-item-section>
                <q-item-label>{{ player.name }}</q-item-label>
                <q-item-label caption
                  >Modifier: {{ player.initiativeModifier || 0 }}</q-item-label
                >
              </q-item-section>
              <q-item-section side>
                <q-input
        ...
        <q-card-actions align="right">
          ...
          <q-btn
            ...
            label="Speichern"
            color="primary"
            @click="savePlayerInitiatives"
          />
        </q-card-actions>
      </q-card>
    </q-dialog>
    <q-dialog v-model="talentDetailDialog">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center">
          <div class="text-h6">{{ selectedTalent?.name }}</div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>

        <q-card-section>
          <div class="text-subtitle2 text-purple q-mb-sm">
            {{
              selectedTalent?.type === "passive"
                ? "Passiv"
                : selectedTalent?.type === "active"
                ? "Aktiv"
                : selectedTalent?.type === "aura"
                ? "Aura"
                : "Spezial"
            }}
            <span v-if="selectedTalent?.level > 1">
              • Level {{ selectedTalent?.level }}</span
            >
          </div>
          ...
          <!-- Effekte-Liste -->
          <div v-if="selectedTalent?.effects" class="q-mt-md">
            <div class="text-subtitle2 q-mb-sm">Effekte:</div>
            <q-list dense bordered separator>
              <q-item
                v-for="(value, key) in formatTalentEffects(
                  selectedTalent?.effects
                )"
                :key="key"
              >
                <q-item-section>
...
const playerCombatants = computed(() => {
  const players = combatStore.sortedCombatants.filter(
    (c) => c.type === "player"
  );
...
const effectiveArmorForHitZone = computed(() => {
  ...
  let armor = 0;
  if (
    defendingCombatant.value?.armorIds &&
    defendingCombatant.value.armorIds.length > 0
  ) {
    const bodyPartMap = {
      Kopf: "head",
      Körper: "torso",
    ...
    const armorData = armorStore.getArmorForBodyPart(
      defendingCombatant.value.armorIds,
      bodyPart
    );
    armor = armorData.rp;
  ...
  const talentBonus = talentStore.calculateEffectiveArmor(
    0,
    defendingCombatant.value?.talents || []
  );
  armor += talentBonus;
...
const actualDefenseSkill = computed(() => {
  ...
  if (defenseType.value === "parry") {
    ...
    const meleeWeapon = defendingCombatant.value.weapons?.find(
      (w) => w.skill === "WS" || w.type === "melee" || w.mag === 0
    );
    ...
    if (meleeWeapon?.special) {
      ...
      if (
        meleeWeapon.special.includes("Unbalanced") ||
        meleeWeapon.special.includes("Unwieldy")
      ) {
...
const ammoUsage = computed(() => {
  ...
  return weaponStore.getAmmoUsage(
    currentWeapon.value.rof,
    selectedFireMode.value
  );
});

const getCurrentCombatantTalents = () => {
  if (!combatStore.currentCombatant?.talents) return [];

  // Nur die ersten 8 kampfrelevanten Talente zeigen
  return combatStore.currentCombatant.talents
    .map((talentInstance) => {
      const talent = talentStore.getTalentById(talentInstance.id);
      if (!talent || !talent.displayInCombat) return null;

      let displayText = talent.name;

      // Kurze Anzeige für stackable Talente
      if (talent.stackable && talentInstance.level > 1) {
        if (talent.id === "flesh_is_weak") {
          displayText = `Fleisch ist schwach ${talentInstance.level}`;
        } else if (talent.id === "unnatural_toughness") {
          displayText = `Unnät. Widerstand x${talentInstance.level + 1}`;
        } else if (talent.id === "unnatural_strength") {
          displayText = `Unnät. Stärke x${talentInstance.level + 1}`;
        } else if (talent.id === "fear_rating") {
          displayText = `Furcht ${talentInstance.level}`;
        } else {
          displayText += ` ${talentInstance.level}`;
        }
      }

      return {
        id: talentInstance.id,
        name: talent.name,
        display: displayText,
        shortDesc: talent.shortDesc,
        description: talent.description,
        type: talent.type,
        level: talentInstance.level || 1,
        effects: talent.effects,
      };
    })
    .filter((t) => t !== null)
    .slice(0, 8); // Maximal 8 anzeigen
};

const showTalentDialog = (talent) => {
  selectedTalent.value = talent;
  talentDetailDialog.value = true;
};

const formatTalentEffects = (effects) => {
  if (!effects) return {};
  ...
  if (effects.armorBonus) {
    formatted[
      "Rüstungsbonus"
    ] = `+${effects.armorBonus} RP pro Level auf alle Trefferzonen`;
  }
  if (effects.toughnessMultiplier) {
    formatted["Widerstand"] = `Bonus wird multipliziert`;
  }
  if (effects.strengthMultiplier) {
    formatted["Stärke"] = `Bonus wird für Nahkampf multipliziert`;
  }
  if (effects.damageReduction) {
    formatted["Schadensreduktion"] =
      effects.damageReduction < 1
        ? "Halbiert normalen Schaden"
        : `-${effects.damageReduction} Schaden`;
  }
  if (effects.immunities) {
    formatted["Immunitäten"] = effects.immunities.join(", ");
  }
  if (effects.fearRating) {
    formatted["Furcht"] = `Verursacht Furcht-Test`;
  }
  if (effects.extraMeleeAttacks) {
    formatted[
      "Extra-Angriffe"
    ] = `+${effects.extraMeleeAttacks} Nahkampfangriffe`;
  }
  if (effects.regenPerRound) {
    formatted["Regeneration"] = `${effects.regenPerRound} HP pro Runde`;
  }
...
const handleEngagementChange = (playerId, newValue) => {
  ...
  if (newValue && !exists) {
    ...
    // Remove player
    tempEngagements.value = tempEngagements.value.filter(
...
const savePlayerInitiatives = () => {
  ...
  playerInitiatives.value.forEach((player) => {
    combatStore.updateCombatant(player.id, {
      initiative: player.rolledInitiative,
    });
  });
...
const performAttack = (weapon, index) => {
  ...
  if (weapon.mag > 0 && weapon.currentAmmo === 0) {
    $q.dialog({
      ...
      $q.notify({
        type: "info",
        message: `${weapon.name} wird nachgeladen (${weapon.reload} Aktion${
          weapon.reload > 1 ? "en" : ""
        })`,
      });
    });
  ...
  if (modes.burst > 0) {
    ...
    if (canBurst) {
      availableFireModes.value.push({
        label: `Salve (${modes.burst})`,
        value: "burst",
      });
    }
  }
  ...
  // Frage nach Ziel
  const enemies = combatStore.sortedCombatants.filter(
    (c) => c.type !== combatStore.currentCombatant.type && c.hp.current > 0
  );
...
const rollAttack = () => {
  ...
  const hasReliable =
    currentWeapon.value.special?.includes("Reliable") || false;
  ...
  if (currentWeapon.value.mag > 0 && currentWeaponIndex.value !== null) {
    ...
    combatStore.useAmmo(
      combatStore.currentCombatant.id,
...
const rollDamageForHit = (index) => {
  ...
  let strengthBonus = 0;
  if (currentWeapon.value.type === "melee") {
    const attacker = combatStore.currentCombatant;
    strengthBonus = Math.floor(attacker.attributes?.ST / 10);

    // Spezialregeln für bestimmte Waffen
    if (currentWeapon.value.special?.includes("Unwieldy")) {
      strengthBonus = Math.floor(strengthBonus / 2); // Halber Stärkebonus
    } else if (
      currentWeapon.value.special?.includes("Power Fist") ||
      currentWeapon.value.name?.includes("Kraftfaust")
    ) {
      strengthBonus = strengthBonus * 2; // Doppelter Stärkebonus
    }
  }
...
const adjustCriticalWounds = (amount) => {
  ...
  combatStore.currentCombatant.criticalWounds = Math.max(
    0,
    combatStore.currentCombatant.criticalWounds + amount
  );
  combatStore.saveCombat();
};
...
const rollDefense = () => {
  ...
  const targetValue = Math.min(
    95,
    Math.max(1, actualDefenseSkill.value + defenseModifier.value)
  );
  defenseSuccess.value = roll <= targetValue;
};

const calculateCoverReduction = () => {
  if (!hitZoneInCover.value || !defendingCombatant.value?.currentCover)
    return 0;
...
const updateCharacterCover = () => {
  ...
  if (
    defendingCombatant.value.currentCover &&
...
const damageCover = () => {
  if (defendingCombatant.value.currentCover) {
    ...
    const remainingCover =
      defendingCombatant.value.currentCover.value -
      defendingCombatant.value.currentCover.damage;
...
const calculateFinalDamage = () => {
  ...
  if (
    hitZoneInCover.value &&
    defendingCombatant.value?.currentCover?.value > 0
  ) {
  ...
  // Verwende die computed property die Talente berücksichtigt
  damage = Math.max(0, damage - toughnessBonus.value);
...
.q-dark .talent-card:hover {
  background: linear-gradient(
    135deg,
    rgba(156, 39, 176, 0.25) 0%,
    rgba(103, 58, 183, 0.25) 100%
  );
}
</style>
